...
def main(argv=None):
    ...
    parser.add_argument('-s', '--entropy', type=int, metavar='ENTROPY',
                        help='Target entropy bits')
    parser.add_argument('-w', '--word-list', type=str, metavar='WORD_LIST',
                        choices=wordlists, help='Use built-in wordlist')
    parser.add_argument('-f', '--word-file', type=str, metavar='WORD_FILE',
                        help='Word file path (one word per line)')
    parser.add_argument('-r', '--random-case', action='store_true',
                        dest='random_case',
                        help='Capitalize first character of each word '
