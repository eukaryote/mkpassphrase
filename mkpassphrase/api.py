...
def mkpassphrase(
    ...
    if not bool(word_file) ^ bool(word_list):
        raise ValueError("exactly one of 'word_list' and " "'word_file' is required")
    ...
    if num_words is None:
        ...
        actual_entropy = calculate_entropy(len(words), num_words, random_case)
    ...
    for _ in range(count):
        passphrase = sample_words(
            words, num_words, delim=delim, random_case=random_case
        )
